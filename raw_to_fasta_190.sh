...
F=~/Documents/${acc}_1.fastq.gz
R=~/Documents/${acc}_2.fastq.gz
...
bowtie2 --local  --score-min G,190,8 -x ~/bowtie_index/New_Ceiba -1 forward_paired.fq.gz  -2 reverse_paired.fq.gz  -U forward_unpaired.fq.gz,reverse_unpaired.fq.gz  -S output.sam 2> $bowtie

samtools view -bS output.sam | samtools sort - bam_sorted
samtools index bam_sorted.bam
samtools mpileup -E -uf ~/bowtie_index/new_ceiba.fna bam_sorted.bam > output.pileup
